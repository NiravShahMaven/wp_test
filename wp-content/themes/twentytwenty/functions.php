...
/* Creating a meta box Custom field for Price add in post*/
add_action( 'add_meta_boxes', 'price_meta_box_add' );
function price_meta_box_add()
{
    add_meta_box(
 		'price-id','Price','price_function','post','normal','default'
 	);
}

add_action( 'add_meta_boxes', 'price_meta_box_add' );

function price_function($post)
{	
...
add_action("save_post","price_save_metabox_data",10,2);
function price_save_metabox_data($post_id,$post){
	...
	if (isset($_POST['price'])) {
		$pub_name=sanitize_text_field($_POST['price']);
	}else{
		 $pub_name='';
	}
	update_post_meta($post_id,"price",$pub_name);
}
/*End custom field*/
